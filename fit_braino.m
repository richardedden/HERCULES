...
%Shift PCho over (phantom is CholineChloride) and remove subtraction errors
BasisSet3(:,15)=circshift(BasisSet3(:,15),[3 0]);
BasisSet3((size(BasisSet3,1)/4+1):end,15)=0;
BasisSet3((size(BasisSet3,1)/4+1):end,3)=0;
BasisSet3(:,11)=circshift(BasisSet3(:,11),[-2 0]);
...
size(Sim_Spectrum)
weights3
weights_init=[[1.5 1.5 10 2 0 2 4 10 0 6 1 10 0 2 23.5 4 0 0 0] zeros(1,8) 0.1 0.5 0];
...
StackPlot(Sim_Spectrum_display,2500000)
hold on
StackPlot(Fit_Spectrum_display,2500000,'r')
spline_vault_display=squeeze(sum(spline_vault,3));
spline_vault_display(:,2:4)=spline_vault_display(:,2:4)*5;
StackPlot(spline_vault_display,2500000,'g')
hold off     
text(400,220,'* 0.2')
