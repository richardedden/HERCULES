file='hercmp_6_2_raw_act.SDAT';
ppm=linspace(0,2000/128,2048)-3.1325;
M.data=SDATreadMEGA(file,2048,20);


M.data=M.data.*conj(repmat(M.data(1,:),[2048 1]))./abs(repmat(M.data(1,:),[2048 1]));

M.data_avg=squeeze(sum(reshape(M.data,[2048 4 5]),3)),
%  for ii=1:4
...
M.spec.ABCD(:,2:2:4)=-M.spec.ABCD(:,2:2:4)
M.spec.H4=permute(hadamard(4)*permute(M.spec.ABCD,[2 1]),[2 1]);
%M.spec.H4(:,3)=-M.spec.H4(:,3);
M.spec.H4(:,1)=M.spec.H4(:,1);
M.spec.ABCD(:,2:2:4)=M.spec.ABCD(:,2:2:4)+0.5;

M.spec.H4(:,1)=M.spec.H4(:,1)-0.2;
M.spec.H4(:,2)=M.spec.H4(:,2)+0.4;
M.spec.H4(:,4)=M.spec.H4(:,4)+0.15;
M.spec.ABCD(:,2:2:4)=M.spec.ABCD(:,2:2:4)-0.55;
subplot(2,1,1)
plot(ppm,M.spec.ABCD);
set(gca,'XLim',[0 4.0]);
