...
weights=[1 1 9 2 3 3 12 3 3 3 1 1 1 1 4 1 27 9 9 3 3 1 1 1 4 1 0.1];
...
weights_init=[weights zeros(1,8) 0.1 0.5];

        lb = [zeros(1,n_metabolites) ones(1,8)*(-1)  0.01 0]; %NP; our bounds are 0.03 less due to creatine shift
        ub = [ones(1,n_metabolites)*20 ones(1,8) 1 1];
...
% 
    ...
    spline2=fit(ppm',Residuals((1:length(ppm))+length(ppm))','smoothingspline','SmoothingParam',0.5);
...
%     
    ...
    plot(ppm,spline_vault(:,1,ii),ppm,Residuals(1:length(ppm)));
...
Fit_Spectrum2=reshape(Hercules_model(Fitted_weights(ii,:),BasisSet),[length(ppm),4]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 
...
StackPlot(Sim_Spectrum_display,200)
hold on
StackPlot(Fit_Spectrum_display,200,'r')
hold off     
text(400,220,'* 0.2')
